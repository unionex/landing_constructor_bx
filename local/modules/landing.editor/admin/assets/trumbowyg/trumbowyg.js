...
(function (navigator, window, document, $) {
            ...
            imgDblClickHandler: function () {
                ...
                t.openModalInsert(t.lang.insertImage, {
					...
					align: {
						label: 'Выравнивание',
						type: 'select',
						value: ['bottom', 'left', 'middle', 'right', 'top']
					}
                }, function (v) {
                    if (v.src !== base64) {
                        $img.attr({
    ...
    Trumbowyg.prototype = {
        ...
        semanticCode: function (force, full, keepRange) {
            ...
            if (t.o.semantic) {
				t.semanticTag('b', 'span style="font-weight:bold;"');
				t.semanticTag('i', 'em');
        ...
        openModalInsert: function (title, fields, cmd) {
            ...
            $.each(fields, function (fieldName, field) {
                ...
                if(textarea) {
					...
					html += '<div class="tarea-infos"><label><span class="' + prefix + 'input-infos"><span>' +
						((!l) ? (lg[fieldName] ? lg[fieldName] : fieldName) : (lg[l] ? lg[l] : l)) +
						'</span></span><textarea name="' + n + '" ' + attr + '>' + (field.value || '').replace(/"/g, '&quot;') + '</textarea></label></div>';

				} else if(field.type === 'select') {
                    html += '<label><select>';
                    $.each(field.value, function (index, value) {
                        html += '<option value="' + index + '">' + value + '</option>';
					});
                    html += '</select><span class="' + prefix + 'input-infos"><span>' + ((!l) ? (lg[fieldName] ? lg[fieldName] : fieldName) : (lg[l] ? lg[l] : l)) + '</span></span></label>';
                } else {

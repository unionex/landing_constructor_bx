...
function HtmlWhitelistedSanitizer(tags, css, urls) {
    ...
    if (this.allowedTags == null) {
        ...
        var globalAttributes = {
			...
			'alt': unconstrainted,
			'rel': unconstrainted,
			'content': unconstrainted,
			'frameborder': unconstrainted,
			'allowfullscreen': unconstrainted,
			'count': unconstrainted,
			'get_param': unconstrainted
        };
        ...
        this.allowedTags = {
            ...
            'rewrite_content': globalAttributes,
			'iframe': globalAttributes,
            'tooltip': globalAttributes,
			'textcat': globalAttributes,
			'textpopup': globalAttributes
        };
    }
...
HtmlWhitelistedSanitizer.prototype.sanitizeNode = function(node) {
    ...
    if (node_name == '#comment') {
		// пропускам эти спец комментарии
        if (('noindex' == node.nodeValue) || ('/noindex' == node.nodeValue)){
			return node;
	    }
		// остальные комментарии убираем

...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $dir = '/upload/landing.editor/temp/';
    ...
    protected function get_file_name($name, $content_range) {
        $name = $this->bitrixTranslite($name);
	...
	public function get()
	{
		...
		$file->path = $upload['tmp_name'];
		$file->name = basename($upload['name']);
		$file->url  = $this->get_download_url($file->name);
		$files = $this->bitrixSave([0 => $file]);
		$response = array($this->options['param_name'] => $files);

		return $this->generate_response($response, true);
	}
    ...
    protected function bitrixSave($files) {
        ...
        foreach ($files as $k => $file) {
            ...
            $aFile['MODULE_ID'] = 'landing.editor';
			$bitrixId = \CFile::SaveFile($aFile, 'content');
			if ($bitrixId) {
				$checkErr = \CFile::CheckImageFile($aFile, 0, 0, 0);
				if (empty($checkErr) && !empty($this->options['bitrix_resize'])) {
    ...
    protected function downloadImage($url){
        ...
        $image = false;
        if (empty($checkErr)) {
            $bitrixId = \CFile::SaveFile($aFile, 'landing.editor');
            if ($bitrixId) {
